...
printf '%s' "${RESTORE_DATABASE:-}" | grep -Eq '^forgeflow_preview_restore_[A-Za-z0-9_]+$' ||
  { echo "RESTORE_DATABASE must match forgeflow_preview_restore_[A-Za-z0-9_]+" >&2; exit 2; }
if [ "${CONFIRM_RESTORE:-}" != "restore-personal-preview-drill" ]; then
  echo "CONFIRM_RESTORE must equal restore-personal-preview-drill" >&2
  exit 2
fi

backup_name="$(basename "${BACKUP_FILE:-}")"
printf '%s' "$backup_name" | grep -Eq '^forgeflow-preview-[0-9]{8}T[0-9]{6}Z\.dump$' ||
  { echo "BACKUP_FILE must use the canonical personal preview UTC backup name" >&2; exit 2; }
...
manifest_database="$(sed -n 's/^databaseName=//p' "$manifest_file")"
...
if [ "$manifest_schema" != "forgeflow.preview-backup/v1" ] || [ "$manifest_backup" != "$backup_name" ] || [ "$manifest_database" != "forgeflow" ] || [ "$manifest_checksum" != "$actual_checksum" ] || [ "$manifest_size" != "$actual_size" ]; then
...
printf 'Personal preview restore drill succeeded into %s at migration %s\n' "$RESTORE_DATABASE" "$restored_migration"
