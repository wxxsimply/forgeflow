...
temporary="/backups/forgeflow-preview-${timestamp}.dump.tmp"
target="/backups/forgeflow-preview-${timestamp}.dump"
...
{
  printf 'schemaVersion=forgeflow.preview-backup/v1\n'
  printf 'backupFile=%s\n' "$backup_name"
  printf 'createdAt=%s\n' "$timestamp"
  printf 'databaseName=forgeflow\n'
...
find /backups -type f \( -name 'forgeflow-preview-*.dump' -o -name 'forgeflow-preview-*.dump.sha256' -o -name 'forgeflow-preview-*.dump.manifest' \) -mtime "+$retention_days" -delete
