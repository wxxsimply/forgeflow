...
记录日期：2026-09-15；基线：PR #67，`007bf2ed58f50245db6f4ea5cf565adf01607aca`。

入口是 `scripts/preview_demo_real.py`。它用于准备、核对、记录批准，并在全部保护条件满足后执行一次可能收费的 DeepSeek 请求。密钥只从仓库及任务目录之外的受控文件读取；`test` 执行候选代码时仍须单独批准，并使用已有无网络容器沙箱。
...
| `send` | 使用已批准的精确计划发起唯一一次请求 | 调用前持久化请求次数与最高费用预留；成功或未知结果都禁止重发 |
...
新增受控凭据与单次发送检查后，完整快速集合共 221 项：219 项通过，2 项因当前 Windows 平台不适用而跳过，约 23.55 秒。

测试全部使用临时公开模板、合成任务/价格/响应和假密钥；网络发送及真实沙箱默认禁止调用。模拟发送覆盖未批准/计划不匹配时零调用、成功时恰好一次调用、未知结果保留预留并阻止重发，以及密钥不进入输出、数据库或传输子进程参数。凭据检查覆盖大小、格式、仓库/任务目录隔离、硬链接、POSIX 权限、合成 Windows ACL 和当前 Windows ACL 查询。

未读取真实凭据、发起真实请求、执行真实候选或修改服务器。当前固定演示任务与命令行路线已经确定，但真正执行前仍须人工核对最新模型/价格/汇率、私有目录和密钥权限、发送范围、一次调用及费用上限；调用后还要核对实际账单，并单独批准候选测试与需求验收。当前停在人工提交 PR，不需要部署。
