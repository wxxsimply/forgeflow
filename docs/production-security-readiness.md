...
- 管理员变更、审批、Promotion/rollback、用户/Session、Artifact 读取、Secret/权限变更写入外部 append-only 审计后端。当前工程适配器使用独立 S3 Object Lock Compliance、KMS、条件创建、签名信封和请求前失败关闭；真实权限/删除保护/查询恢复仍须验收。
