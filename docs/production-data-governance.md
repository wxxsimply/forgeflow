...
未批准 Region、子处理者或跨境路径时，系统必须拒绝对应 Production 配置，不以用户点击普通服务条款替代必要授权。Production 启动时会读取经过 SHA-256 绑定的私有 Policy 文件，校验实际模型端点、Artifact/Audit Region、Trace 端点、子处理者和隐私/条款版本；字段契约和验收顺序见[数据治理配置门禁](production-data-governance-gate.md)。
