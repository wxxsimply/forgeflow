...
- 新增私有计划初始化器和 CI 回归测试：自动绑定完整 HEAD，只写 Git 忽略目录，拒绝覆盖与越界路径，并在落盘前调用现有填充计划预检；没有把任何真实私有值写入仓库。
...
- `scripts/test-stage-9-plan-initializer.ps1`：在干净临时克隆中通过成功生成、完整 HEAD 绑定、无占位符、拒绝覆盖和拒绝越界路径检查。
