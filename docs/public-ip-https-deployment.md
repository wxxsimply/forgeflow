...
旧版预览没有访客自助注册页面；含公开注册接口的版本发布后，访客可在登录页进入注册页。注册只创建普通操作员账号，仍受仓库根目录、Mock 流程和用户数据隔离限制。

- 不要把管理员密码共享给其他人。
- 普通用户应各自注册独立账号，不得共用管理员账号；当前没有邮箱验证或自助密码找回，公开发布前需明确告知测试者。
- 公网 overlay 启用了 Secure Cookie；启用期间只通过 `https://39.102.136.31` 登录，不把 HTTP 8080 当作并行登录入口。
- 个人预览仅为登录处理注册邮箱与密码摘要；仓库、任务及附件仍不得包含其他真实敏感数据，也不承担生产可用性承诺。
