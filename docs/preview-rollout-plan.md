...
PR #53 已人工合并，服务器已在明确授权后部署到 `238bf013e57bcacb4b381093dd3761b760881a87`。部署前 23 项、部署后 28 项只读检查全部通过，API/Web 外部版本一致，HTTPS 登录页返回 200。当前停止点是本轮记录的人工提交与浏览器验收，详见 [实际部署记录](preview-rollout-result.md)。

以下第 1～4 节和第 8 节保存的是 PR #53 提交前的历史交接，不是当前待办；不要重复创建已合并 PR。第 5～7 节保留为操作和回退参考，未来部署须重新核对版本与授权。
