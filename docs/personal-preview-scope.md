...
> 状态：PERSONAL-001 已完成；公开注册规则在包含注册接口的版本发布后生效
>
> 初始边界生效日期：2026-09-22；公开注册候选记录日期：2026-09-26，待发布
>
> 适用对象：个人维护的受限公开预览，不是生产级托管服务

## 1. 用户范围

- 旧版现网只允许项目所有者邀请并单独创建账号，同时最多 5 个活跃测试账号；该限制不适用于公开注册候选版本。
- 含 `POST /api/v1/auth/register` 的版本发布后，访客可公开自助注册。注册接口只能创建 `operator` 普通账号，不能指定或获得管理员角色；注册成功后仍须独立登录。
- 普通账号仅能访问自己的仓库、任务和审批记录；仓库仍限于所有者事先检查过的专用预览目录，注册不授权任意路径或真实模型调用。
- 注册接口按 API 接收端来源限流（反向代理部署下访客可能共享配额），密码使用 Argon2id 哈希。当前没有邮箱验证或自助密码找回；不得将其宣传为生产级身份验证服务。
...
本记录关闭初始 PERSONAL-001。公开注册变更以实际发布版本为准；HTTPS 与 Secret 状态由 docs/personal-preview-readiness.md 单独记录。备份恢复、可观测性与不可信执行边界仍须按各自证据复核。
