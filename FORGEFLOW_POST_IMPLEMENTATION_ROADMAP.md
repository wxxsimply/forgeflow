...
- [x] 首次 Git commit 和 GitHub 主仓库已建立。
- [x] GitHub 分支保护和必需 CI 已启用。
- [x] 30 个真实 fixture commit 已验证。
- [x] 三基线报告已生成并人工签署。
...
统一入口见 `docs/stage-9-acceptance-window-runbook.md`。先复制 `deploy/release/stage-9-acceptance-plan.template.json` 到已被 Git 忽略的 `.forgeflow/acceptance/1.0.0/plan.json`，填入冻结值和私有审批记录 ID，再运行：

```powershell
./scripts/stage-9-acceptance-preflight.ps1 -Plan .forgeflow/acceptance/1.0.0/plan.json
```

不带 `-Plan` 时只运行快速静态契约检查，不联系 Provider、Registry 或 Staging，也不执行任何治理变更。
