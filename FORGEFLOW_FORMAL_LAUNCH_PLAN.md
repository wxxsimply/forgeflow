...
| PERSONAL-005 | 仓库防护回归就绪，待服务器实测 | 最小可观测性与回滚 | P0 | CI 覆盖脱敏观测、私有环境文件与只读回滚计划；仍需所有者授权后启用定时器、连续观测并完成真实应用回滚演练 |
| PERSONAL-006 | 离线决策写入回归就绪，待付费实测 | 候选功能 smoke | P1 | 私有计划、双重付费确认、固定 feature-01、基线/候选严格 GO/NO-GO 判定和原子防覆盖决策写入已由离线测试覆盖；仍需所有者明确授权后运行新 Campaign |
| PERSONAL-007 | 文案与入口改进就绪，待当前候选人工验收 | 受限用户端到端验收 | P1 | 工作台已说明产品用途、当前模拟边界、可体验步骤并为可操作账号提供直接入口；浏览器回归验证文案和角色入口。2026-09-17/18 的“用户容易不清楚平台功能”反馈仍须由真实测试者在同一候选上复核；其余流程仍按 docs/personal-preview-e2e-acceptance.md 留存私有证据 |
| PERSONAL-008 | 仓库拒绝回归就绪，待真实环境验证 | 不可信执行边界 | P0（开放任意仓库前） | 个人预览的 API/Worker 源仓库挂载只读并由 validate 防漂移；Go 回归拒绝工作区及工作目录符号链接逃逸。独立执行面、最小权限、无外网 Sandbox 与真实拒绝测试仍须在环境中验证。未完成前仅允许受控仓库 |
...
6. PERSONAL-007 的工作台已针对“用户容易不清楚平台功能”反馈补充用途说明、模拟范围和开始入口，并有角色回归；合并后按 docs/personal-preview-e2e-acceptance.md 用同一候选完成真实受限用户端到端验收，确认真实测试者可自行理解用途并开始体验。2026-09-17/18 的旧回报因缺少当前候选 SHA，不代替本轮结果。
