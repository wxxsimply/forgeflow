...
> 当前非敏感用户、仓库、数据、模型费用和停止边界统一见 [个人预览边界](docs/personal-preview-scope.md)；HTTPS 与 Secret 证据见 [个人预览就绪记录](docs/personal-preview-readiness.md)；备份与隔离恢复步骤见 [个人预览备份恢复手册](docs/personal-preview-backup-restore.md)；最小观测与回滚见 [个人预览观测回滚手册](docs/personal-preview-operations.md)。受邀者身份、备份内容、观测快照和部署私有记录不得提交 Git。
>
...
> 历史应用镜像基线：`d5d5f5311bd812f511f009cfbc458f52ccbc6a17`（PR #47；已被上述新版本替代，旧镜像保留供回退）
> 当前部署配置：PR #53 版本的基础 Compose 与公网 HTTPS overlay；只更新三项应用环境变量，Caddy、证书卷与数据库保持原状
...
- [x] 脱敏观测、15 分钟定时器和只读回滚计划已准备并接入 CI；真实连续观测和应用回滚演练仍待所有者单独授权，不计为 PERSONAL-005 完成。
...
- [x] SNI 修复代码已通过 PR #50 人工合并。
- [ ] 重新核对服务器工作区与已合并提交的对应关系；此项不以 GitHub 合并代替线上验证。
