...
> 2026-09-14 补充：PR #50（SNI）、#51（中文界面）和 #52（预览可靠性）代码已合并。以下旧部署版本属于历史记录，不能视为当前线上确认；本轮 SSH 连接被关闭，尚未核实线上是否同步。近期更新以 [阶段 C 操作单](docs/preview-rollout-plan.md) 为准，禁止跳过服务器变更确认。

> 生效日期：2026-09-09  
> 当前目标：`v0.1.0-preview.1` 个人公开预览（公网 HTTPS 已上线，待人工登录验收）
> 未来目标：正式 `v1.0.0`，暂不执行  
> 服务器：`39.102.136.31`，Ubuntu 22.04.5 LTS，公网入口 `https://39.102.136.31`
> 历史应用镜像基线：`d5d5f5311bd812f511f009cfbc458f52ccbc6a17`（PR #47；当前实际版本待重新核对）
> 历史部署配置基线：`3ff97639df45728508ab814432f345daeab0cba4`（PR #49）加当时已部署的 `default_sni` 补丁；代码后续已通过 PR #50 合并，当前线上配置待重新核对
