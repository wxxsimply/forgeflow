...
import { nodeLabel, eventLabel, errorMessage } from '../utils/labels';
...
export function RunDetailPage() {
  ...
  if (runQuery.error) {
    ...
    return <div className="page"><PageState tone="danger" title={missing ? '任务不存在或不可访问' : '无法加载任务'} detail={missing ? '资源可能已删除，或者当前账号没有访问权限。' : '请检查服务连接后重试。'} action={<Link className="secondary-button" to="/runs">返回列表</Link>} /></div>;
  }
  if (!run) return <div className="page"><PageState tone="danger" title="任务响应为空" detail="请刷新页面重试。" /></div>;

  function runAction(action: 'pause' | 'resume' | 'cancel') {
    const labels = { pause: '暂停', resume: '恢复', cancel: '取消' };
    if (!window.confirm(`确认${labels[action]}此任务？`)) return;
  ...
  return <div className="page run-detail">
    ...
    <div className="detail-heading">
      <div><span className="eyebrow">任务 · {run.runId.slice(0, 8)}</span><h1>{run.task}</h1><p>{shortPath(run.repositoryPath)} · {run.baseRevision}</p></div>
    ...
    {lifecycle.error && <div className="form-error" role="alert">{errorMessage(lifecycle.error)}</div>}
    {run.pendingApproval && <div className="approval-banner"><div><span className="eyebrow">人工审批</span><strong>{run.pendingApproval.reason}</strong></div><Link className="primary-button primary-button--fit" to={`/approvals/${run.pendingApproval.approvalId}`}>检查审批</Link></div>}
    <RunSubnav runId={runId} />
    <section className="metric-grid" aria-label="任务摘要">
      <Metric label="当前节点" value={nodeLabel(run.currentNodeId)} mono /><Metric label="运行时长" value={formatDuration(run.createdAt, run.updatedAt)} /><Metric label="版本" value={`v${run.version}`} mono /><Metric label="预计成本" value={formatCost(run)} />
    </section>
    <div className="detail-grid">
      <section className="panel graph-panel"><div className="panel-heading"><div><span className="eyebrow">工作流进度</span><h2>执行轨迹</h2></div><span className={`stream-state stream-state--${terminal.has(run.status) ? 'complete' : streamState}`}><i />{terminal.has(run.status) ? '已结束' : streamLabel(streamState)}</span></div><GraphProgress run={run} />{run.error && <div className="run-error" role="alert"><strong>{run.error.code}</strong><span>{run.error.message}</span></div>}</section>
      <section className="panel timeline-panel"><div className="panel-heading"><div><span className="eyebrow">执行事件</span><h2>事件时间线</h2></div><span className="event-count">{eventsQuery.data?.items.length ?? 0} 条事件</span></div>{eventsQuery.isPending ? <LoadingRows count={4} /> : eventsQuery.error ? <PageState tone="danger" title="事件加载失败" detail="实时连接会继续尝试恢复。" /> : <Timeline items={eventsQuery.data.items} />}</section>
    </div>
  </div>;
}
...
function LifecycleActions({ run, busy, act }: { run: Run; busy: boolean; act: (action: 'pause' | 'resume' | 'cancel') => void }) {
  ...
  return <div className="lifecycle-actions">{run.status === 'paused' ? <button className="secondary-button" disabled={busy} onClick={() => act('resume')}>恢复</button> : <button className="secondary-button" disabled={busy} onClick={() => act('pause')}>暂停</button>}<button className="danger-button" disabled={busy} onClick={() => act('cancel')}>取消任务</button></div>;
}
...
function GraphProgress({ run }: { run: Run }) { const completed = new Set(run.completedNodeIds ?? []); const nodes = ['start', 'planner', 'validate-plan', 'plan-approval', 'prepare-workspace', 'implement', 'evaluate', 'judge', 'end']; return <ol className="graph-flow">{nodes.map((node) => <li key={node} className={completed.has(node) ? 'complete' : run.currentNodeId === node ? 'current' : ''}><i /><span>{nodeLabel(node)}</span></li>)}</ol>; }
function Timeline({ items }: { items: SequencedEvent[] }) { if (items.length === 0) return <PageState title="暂无事件" detail="后台执行器开始处理后，追加事件会显示在这里。" />; return <ol className="timeline">{[...items].reverse().map((item) => <li key={item.sequence}><span className="timeline__dot" /><div><span className="timeline__meta"><b>#{item.sequence}</b>{formatDateTime(item.event.createdAt)}</span><strong>{eventTitle(item.event.type)}</strong><p>{item.event.message}</p>{item.event.nodeId && <code title={item.event.nodeId}>{nodeLabel(item.event.nodeId)}</code>}</div></li>)}</ol>; }
function eventTitle(type: string): string { return eventLabel(type); }
