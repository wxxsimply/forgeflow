...
import { actionLabel, riskLabel } from '../utils/labels';
...
export function ApprovalsPage() {
  ...
  return <div className="page">
    <div className="page-heading"><div><span className="eyebrow">人工审批</span><h1>审批中心</h1><p>审查 智能体计划和高风险工具调用，所有决定都会进入审计日志。</p></div><div className="page-heading__meta"><strong>{query.data?.items.length ?? 0}</strong><span>当前筛选</span></div></div>
    ...
    {query.isPending ? <LoadingRows /> : query.error ? <PageState tone="danger" title="无法加载审批" detail="请检查服务连接后重试。" /> : query.data.items.length === 0 ? <PageState title="此筛选下没有审批" detail="当任务到达人工检查点时，审批会显示在这里。" /> : <div className="approval-list">
      {query.data.items.map(({ request, runVersion }) => <Link className="approval-row" to={`/approvals/${request.approvalId}`} key={request.approvalId}>
        <span className={`risk risk--${request.risk}`}>{riskLabel(request.risk)}</span>
        <span><strong>{actionText(request.actionType)}</strong><small>{request.reason}</small></span>
        <span className="mono">任务 {request.runId.slice(0, 8)} · v{runVersion}</span>
...
function actionText(action: string): string { return actionLabel(action); }
